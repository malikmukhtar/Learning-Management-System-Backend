...
export const create = async (req, res) => {
  try {
    ...
    if (alreadyExist) return res.status(400).send("Title already taken");
    ...
    console.log(err);
    return res.status(400).send("Course create Failed. Tyr Again");
  }
};
...
export const uploadVideo = async (req, res) => {
  try {
    ...
    if (!video) return res.status(400).send("No Video Selected");
...
export const paidEnrollment = async (req, res) => {
  try {
    ...
    console.log("PAID ENROLLMENT ERR1111", err.message);
    return res.status(400).send("Enrollment create failed");
  }
};
...
export const listCompleted = async (req, res) => {
  try {
    ...
    console.log("list completed err", err);
  }
};
